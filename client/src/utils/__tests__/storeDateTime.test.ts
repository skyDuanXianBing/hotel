...
const assertValidFallbackTimeZone = (): void => {
  ...
  assertEquals(getDefaultStoreTimeZone(), 'Asia/Tokyo', 'default timezone should fall back to Tokyo')
  assertEquals(resolveStoreTimeZone('bad/timezone'), 'Asia/Tokyo', 'invalid store timezone should fall back to Tokyo')
  assertEquals(resolveStoreTimeZone(' Asia/Tokyo '), 'Asia/Tokyo', 'valid timezone should be trimmed')
  assertEquals(
...
const assertYmdArithmeticUsesUtcDateOnlyMath = (): void => {
  ...
  assertEquals(parsed.toISOString(), '2026-03-08T00:00:00.000Z', 'YMD parsing should use UTC midnight')
  assertEquals(addDaysToYmd('2026-03-08', 1), '2026-03-09', 'adding days should ignore local DST jumps')
