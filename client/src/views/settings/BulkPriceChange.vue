...
const roomTypeOptions = [
  { id: 1, name: '\u5927\u5e8a\u623f', channelMapping: '\u53cc\u4eba\u95f4', basePrice: 140 },
  { id: 2, name: 'aa', channelMapping: '\u5927\u5e8a\u623f', basePrice: 12 },
  { id: 3, name: '\u5927\u5e8a\u623f', channelMapping: '\u5927\u5e8a\u623f', basePrice: 140 },
  { id: 4, name: '\u5927\u5385', channelMapping: 'aa', basePrice: 12 },
]
...
watch(
  ...
  (newRoomTypes) => {
    ...
    const newData = newRoomTypes.map((roomTypeId) => {
      const roomType = roomTypeOptions.find((rt) => rt.id === roomTypeId)
      ...
      return {
        ...
        currentPrice: existingData?.currentPrice || roomType.basePrice || 0,
        currentWeekendPrice: existingData?.currentWeekendPrice || roomType.basePrice || 0,
...
const onSelectAllRoomTypesChange = (checked: boolean) => {
  if (checked) {
    selectedRoomTypes.value = roomTypeOptions.map((rt) => rt.id)
  } else {
    selectedRoomTypes.value = []
  }
}

// 房型选择变化处理 (注释掉，因为暂时未使用)
// const onRoomTypeChange = () => {
//   selectAllRoomTypes.value = selectedRoomTypes.value.length === roomTypeOptions.length
// }
