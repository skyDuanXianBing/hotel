...
const props = defineProps<{ settingsId: number }>()
...
const savingMonthly = ref(false)
const monthlyLoading = ref(false)
const exportingType = ref<ManagedOperationDocumentType | null>(null)
const availableRooms = ref<ManagedOperationRoom[]>([])
const settings = reactive<ManagedOperationSettings>(createDefaultManagedOperationSettings())
...
const storedAirbnbFileName = ref('')
const storedBookingFileName = ref('')
const monthlyPersisted = ref(false)
const monthlySnapshot = ref('')
const lastSuggestion = ref<ManagedOperationNumberSuggestion | null>(null)

const issueDayDialogVisible = ref(false)
const invoiceIssueDayValue = ref(9)
const receiptIssueDayValue = ref(10)
const issueDaySaving = ref(false)
...
const serializeMonthly = () =>
  JSON.stringify({
    ...runRequest,
    fees: runRequest.fees,
    storedAirbnb: storedAirbnbFileName.value,
    storedBooking: storedBookingFileName.value,
  })

const monthlyUnsaved = computed(
  () => !monthlyPersisted.value || serializeMonthly() !== monthlySnapshot.value,
)
...
const missingFiles = computed(
  () =>
    (!airbnbFile.value && !storedAirbnbFileName.value) ||
    (!bookingFile.value && !storedBookingFileName.value),
)
...
const replaceSettings = (value: ManagedOperationSettings, persisted: boolean) => {
  Object.assign(settings, createDefaultManagedOperationSettings(), value)
...
const loadStampPreview = async () => {
  ...
  try {
    const blob = await getManagedOperationStamp(props.settingsId)
...
const loadSettings = async () => {
  loading.value = true
  try {
    const response = await getManagedOperationSettings(props.settingsId)
...
const handleSaveSettings = async () => {
  ...
  saving.value = true
  try {
    const response = await saveManagedOperationSettings(props.settingsId, {
      ...settings,
...
const handleStampChange = async (file: UploadFile) => {
  ...
  try {
    const result = await uploadManagedOperationStamp(props.settingsId, raw)
...
const applyMonthlyData = (data: ManagedOperationMonthlyData) => {
  runRequest.settlementMonth = data.settlementMonth
  runRequest.invoiceNumber = data.invoiceNumber
  runRequest.invoiceDate = data.invoiceDate
  runRequest.paymentDueDate = data.paymentDueDate
  runRequest.receiptNumber = data.receiptNumber
  runRequest.receiptDate = data.receiptDate
  runRequest.note = data.note
  runRequest.fees.splice(0, runRequest.fees.length, ...data.fees.map((fee) => ({ ...fee })))
  storedAirbnbFileName.value = data.airbnbFileName
  storedBookingFileName.value = data.bookingFileName
  monthlyPersisted.value = data.persisted
  monthlySnapshot.value = serializeMonthly()
}

const loadMonthlyData = async () => {
  if (!runRequest.settlementMonth) return
  monthlyLoading.value = true
  lastSuggestion.value = null
  try {
    const data = await getManagedOperationMonthlyData(props.settingsId, runRequest.settlementMonth)
    applyMonthlyData(data)
    if (!data.persisted) {
      try {
        const suggestion = await getManagedOperationDocumentNumbers(
          props.settingsId,
          runRequest.settlementMonth,
        )
        lastSuggestion.value = suggestion
        runRequest.invoiceNumber = suggestion.invoiceNumber
        runRequest.receiptNumber = suggestion.receiptNumber
        runRequest.invoiceDate = suggestion.invoiceDate
        runRequest.receiptDate = suggestion.receiptDate
        monthlySnapshot.value = serializeMonthly()
      } catch (suggestionError) {
        // 编号建议失败不阻断月度数据展示，用户仍可手动填写
        console.warn('Failed to suggest document numbers:', suggestionError)
      }
    }
  } catch (error) {
    console.error('Failed to load managed operation monthly data:', error)
    ElMessage.error(t('managedOperation.messages.loadFailed'))
  } finally {
...
watch(
  () => runRequest.settlementMonth,
  (month, previous) => {
    if (month && month !== previous) loadMonthlyData()
  },
)

const invalidFee = computed(() =>
  runRequest.fees.some((item) => !item.description.trim() || Number(item.amountGross) < 0),
)
...
const handleSaveMonthly = async () => {
  ...
  if (invalidFee.value) {
    ElMessage.warning(t('managedOperation.validation.fee'))
    return
  }
  savingMonthly.value = true
  try {
    const data = await saveManagedOperationMonthlyData(
      props.settingsId,
      cleanRunRequest(),
      airbnbFile.value,
      bookingFile.value,
    ...
    applyMonthlyData(data)
    ElMessage.success(t('managedOperation.monthly.saved'))
  } catch (error) {
    console.error('Failed to save managed operation monthly data:', error)
    ElMessage.error(t('managedOperation.monthly.saveFailed'))
  } finally {
    savingMonthly.value = false
  }
}

const openIssueDayDialog = () => {
  invoiceIssueDayValue.value = settings.invoiceIssueDay || 9
  receiptIssueDayValue.value = settings.receiptIssueDay || 10
  issueDayDialogVisible.value = true
}

const handleSaveIssueDay = async () => {
  if (issueDaySaving.value) return
  issueDaySaving.value = true
  try {
    const response = await updateManagedOperationIssueDay(
      props.settingsId,
      invoiceIssueDayValue.value,
      receiptIssueDayValue.value,
    )
    settings.invoiceIssueDay = response.settings.invoiceIssueDay
    settings.receiptIssueDay = response.settings.receiptIssueDay
    const savedSettings = JSON.parse(
      savedSettingsSnapshot.value || '{}',
    ) as Partial<ManagedOperationSettings>
    savedSettingsSnapshot.value = JSON.stringify({
      ...savedSettings,
      invoiceIssueDay: settings.invoiceIssueDay,
      receiptIssueDay: settings.receiptIssueDay,
    })
    issueDayDialogVisible.value = false
    ElMessage.success(t('managedOperation.issueDay.saved'))

    // 本月数据未保存且编号/日期仍停留在上一次建议值时，按新固定日重新建议
    if (!monthlyPersisted.value && runRequest.settlementMonth) {
      const previous = lastSuggestion.value
      const invoiceUntouched =
        !runRequest.invoiceNumber || runRequest.invoiceNumber === previous?.invoiceNumber
      const receiptUntouched =
        !runRequest.receiptNumber || runRequest.receiptNumber === previous?.receiptNumber
      if (invoiceUntouched || receiptUntouched) {
        try {
          const suggestion = await getManagedOperationDocumentNumbers(
            props.settingsId,
            runRequest.settlementMonth,
          ...
          console.warn('Failed to refresh document numbers:', suggestionError)
        }
      }
    }
  } catch (error) {
    console.error('Failed to update issue day:', error)
    ElMessage.error(t('managedOperation.issueDay.saveFailed'))
  } finally {
    issueDaySaving.value = false
  }
}

const addFee = () => {
  runRequest.fees.push({ feeType: 'DEDUCTION', description: '', amountGross: 0 })
}

const removeFee = (index: number) => {
  runRequest.fees.splice(index, 1)
}
...
<template>
  <div v-loading="loading" class="property-detail">
    <header class="detail-header">
      <el-button text :icon="ArrowLeft" @click="emit('back')">
        {{ t('managedOperation.actions.backToGrid') }}
      </el-button>
      <h2 class="detail-title" :title="settings.propertyName">{{ settings.propertyName }}</h2>
    ...
    <el-card v-loading="monthlyLoading" shadow="never" class="section-card">
      <template #header>
        <div class="card-heading">
          <div>
          ...
          <div class="monthly-actions">
            <el-tag v-if="monthlyUnsaved" type="warning" effect="plain">
              {{ t('managedOperation.monthly.unsaved') }}
            </el-tag>
            <el-tooltip :content="t('managedOperation.issueDay.title')" placement="top">
              <el-button :icon="Setting" circle @click="openIssueDayDialog" />
            </el-tooltip>
            <el-button type="primary" plain :loading="savingMonthly" @click="handleSaveMonthly">
              {{ t('managedOperation.actions.saveMonthly') }}
            </el-button>
          </div>
        </div>
      </template>
      ...
      <div class="upload-grid">
        <div class="upload-panel">
          ...
          <div v-if="!airbnbFile && storedAirbnbFileName" class="stored-file">
            <el-icon><Document /></el-icon>
            <span class="stored-file__name" :title="storedAirbnbFileName">
              {{ storedAirbnbFileName }}
            </span>
            <el-tag size="small" type="success" effect="plain">
              {{ t('managedOperation.monthly.storedFile') }}
            </el-tag>
          </div>
        </div>
        ...
        <div class="upload-panel">
          ...
          <div v-if="!bookingFile && storedBookingFileName" class="stored-file">
            <el-icon><Document /></el-icon>
            <span class="stored-file__name" :title="storedBookingFileName">
              {{ storedBookingFileName }}
            </span>
            <el-tag size="small" type="success" effect="plain">
              {{ t('managedOperation.monthly.storedFile') }}
            </el-tag>
      ...
      <p v-if="storedAirbnbFileName || storedBookingFileName" class="stored-hint">
        {{ t('managedOperation.monthly.storedHint') }}
      </p>
    </el-card>
    ...
    <el-card shadow="never" class="section-card">
      <template #header>
        <div class="card-heading">
          <div>
            <h2>{{ t('managedOperation.sections.fees') }}</h2>
            <p>{{ t('managedOperation.sections.feesHint') }}</p>
          </div>
          <el-button :icon="Plus" @click="addFee">
            {{ t('managedOperation.actions.addFee') }}
          </el-button>
        </div>
      </template>
      ...
      <div v-if="runRequest.fees.length" class="fee-list">
        <div v-for="(item, index) in runRequest.fees" :key="index" class="fee-row">
          <el-select v-model="item.feeType" class="fee-type-select">
            <el-option :label="t('managedOperation.feeType.deduction')" value="DEDUCTION" />
            <el-option :label="t('managedOperation.feeType.credit')" value="CREDIT" />
          </el-select>
          <el-input
            v-model="item.description"
            maxlength="100"
            :placeholder="t('managedOperation.placeholders.feeDescription')"
          />
          <el-input-number
          ...
          <el-button
            ...
            type="danger"
            :icon="Delete"
            :aria-label="t('managedOperation.actions.removeFee')"
            @click="removeFee(index)"
          />
        </div>
      </div>
      <el-empty v-else :image-size="64" :description="t('managedOperation.noFees')" />
    ...
    <template v-if="preview">
      ...
      <section class="summary-section">
        ...
        <div class="money-grid">
          ...
          <article>
            <span>{{ t('managedOperation.summary.fees') }}</span>
            <strong>
              {{
                formatMoney(
    ...
    <el-dialog
      v-model="issueDayDialogVisible"
      :title="t('managedOperation.issueDay.title')"
      width="440px"
    >
      <p class="issue-hint">{{ t('managedOperation.issueDay.hint') }}</p>
      <el-form-item :label="t('managedOperation.fields.invoiceIssueDay')">
        <el-input-number v-model="invoiceIssueDayValue" :min="1" :max="28" :precision="0" />
      </el-form-item>
      <el-form-item :label="t('managedOperation.fields.receiptIssueDay')">
        <el-input-number v-model="receiptIssueDayValue" :min="1" :max="28" :precision="0" />
      </el-form-item>
      <template #footer>
        <el-button @click="issueDayDialogVisible = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" :loading="issueDaySaving" @click="handleSaveIssueDay">
          {{ t('common.save') }}
        </el-button>
      </template>
    </el-dialog>
