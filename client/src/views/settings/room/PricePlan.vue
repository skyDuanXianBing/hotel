    ...
    const roomTypesWithPrices = await Promise.all(
      roomTypesData.map(async (roomType: RoomTypeDTO) => {
        ...
        const pricePlans = pricePlansData.map((pp) => ({
          ...
          prices: {
            mon: pp.monPrice || roomType.monPrice || 0,
            tue: pp.tuePrice || roomType.tuePrice || 0,
            wed: pp.wedPrice || roomType.wedPrice || 0,
            thu: pp.thuPrice || roomType.thuPrice || 0,
            fri: pp.friPrice || roomType.friPrice || 0,
            sat: pp.satPrice || roomType.satPrice || 0,
            sun: pp.sunPrice || roomType.sunPrice || 0,
          },
...
const handleConfirmEditRate = async () => {
  ...
  try {
    ...
    await updateRoomTypePricePlan(plan.id, userStore.currentUser!.id, {
      monPrice: dailyPrices.mon,
      tuePrice: dailyPrices.tue,
      wedPrice: dailyPrices.wed,
      thuPrice: dailyPrices.thu,
      friPrice: dailyPrices.fri,
      satPrice: dailyPrices.sat,
      sunPrice: dailyPrices.sun,
