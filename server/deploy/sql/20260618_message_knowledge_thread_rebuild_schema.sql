-- Manual schema alignment for the 2026-06-18 message knowledge thread rebuild.
--
-- Run this manually only after taking a database backup and before enabling or
-- rerunning the thread knowledge extractor. This script intentionally contains
-- schema DDL only: it does not delete, truncate, merge, or clean old data.
--
-- Target database: the current DATABASE() selected by the SQL client.
-- Expected engine: MySQL/MariaDB.
...
-- Deliberately no DELETE/TRUNCATE here.
-- Any old-data cleanup must be reviewed and executed as a separate explicit runbook.
