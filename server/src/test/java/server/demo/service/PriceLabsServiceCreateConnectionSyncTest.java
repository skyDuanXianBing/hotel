...
class PriceLabsServiceCreateConnectionSyncTest {
    ...
    void createConnection_shouldSyncListingRatePlanAndCalendar() {
        ...
        when(connectionRepository.findByRoomTypeIdAndPricePlanId(34L, 10L))
                .thenReturn(Optional.empty());
        when(roomTypeRepository.findById(34L)).thenReturn(Optional.of(roomType));
        when(pricePlanRepository.findById(10L)).thenReturn(Optional.of(pricePlan));
